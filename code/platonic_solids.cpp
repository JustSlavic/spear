...
{
    float32 a = 0.5773502691896258; // 1 / sqrt(3)
...
cpu_mesh make_ico_sphere(memory_allocator temp_allocator, memory_allocator allocator)
{
    ...
    auto index_map = temp_allocator.allocate_array<uint32>(square(index_map_dim));
    index_map.resize(square(index_map_dim));

    printf("index map (size=%llu, capacity=%llu)\n", index_map.size(), index_map.capacity());
    ...
    for (int i = 0; i < ARRAY_COUNT(ibo_data) / 3; i+=3)
    {
        uint32 iis[4] =
        {
            ibo_data[i + 0],
    ...
    printf("-------- index map --------\n");
    for (int i = 0; i < index_map_dim; i++)
    {
        for (int j = 0; j < index_map_dim; j++)
        {
            printf("%4u", index_map[i * index_map_dim + j]);
        }
        printf("\n");
    }
    printf("---------------------------\n");
    ...
    {
        vbo_data[vbo_count++] = new_vertices[i].x;
        vbo_data[vbo_count++] = new_vertices[i].y;
        vbo_data[vbo_count++] = new_vertices[i].z;
        vbo_data[vbo_count++] = new_vertices[i].x;
        vbo_data[vbo_count++] = new_vertices[i].y;
        vbo_data[vbo_count++] = new_vertices[i].z;
    }
